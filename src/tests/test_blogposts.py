...
from app.api.models import MemoDB

# ----------------------------------------------------------------------------------------------
# use the pytest monkeypatch fixture to mock out the crud.post operation: 
def test_create_memo(test_app, monkeypatch):
...
# ----------------------------------------------------------------------------------------------
def test_create_memo_invalid_json(test_app):
...
def test_update_blogpost(test_app, monkeypatch):
    ...
    async def mock_get(id) -> MemoDB:
        bp = MemoDB(id=id, owner=1, title="someone", description="someone else")
        return bp
...
def test_remove_blogpost(test_app, monkeypatch):
    ...
    async def mock_get(id) -> MemoDB:
        td = MemoDB(id=1, owner=1, title="something", description="something else")
        return td
