...
from app.api.models import UserInDB, AiChatDB, AiChatCreate, AiChatCreateResponse, ProjectDB, TagDB
...
async def create_aiChatExchange(payload: AiChatCreate, 
    ...
    prePrompt = '''You are a highly knowledgable attorney. You only answer truthfully, as an attorney is required.
    ...
    If the question is about a state other than California, make sure to say their state laws may vary.
    
    A potential client asks: '''
    aiResponse = openai.Completion.create(model=MODEL, 
                                          prompt=prePrompt + payload.prompt,
                                          temperature=0,
                                          max_tokens=900,
                                          # top_p=1,
    ...
    #
    rawReply = aiResponse # json.dumps(aiResponse.__dict__)
    #
    log.info(f"create_aiChatExchange: prompt   '{payload.prompt}'")
    log.info(f"create_aiChatExchange: rawReply '{rawReply}'")
    
    aichatid = await crud.post_aiChat(payload, rawReply, current_user)
