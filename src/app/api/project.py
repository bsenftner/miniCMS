...
from app.api.models import UserInDB, TagDB, basicTextPayload, ProjectRequest, ProjectSchema, ProjectDB
...
async def create_project(payload: ProjectRequest, 
    ...
    log.info(f"create_project: here!")
    ...
    if tagid is not None:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="A Project or Tag with this name already exists.")
    else:
...
async def read_all_projects(current_user: UserInDB = Depends(get_current_active_user)) -> List[ProjectDB]:
    ...
    tagList = await crud.get_all_projects(current_user)
    return tagList
